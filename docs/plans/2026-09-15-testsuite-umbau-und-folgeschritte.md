...
**Vorab, PR 3.0 (entschieden 26.09.):** die flächige Umstellung der reinen Sourcing-Helfer auf `source_module()` (327 Aufrufstellen, elf Helfernamen (der zwölfte, `source_zuordnung` für das Skript, bleibt)), die aus Stufe 2 herausgenommen wurde. Reines Refactoring ohne Erwartungsänderung, Nachweis per identischer Multimenge wie Task 7; danach gilt der README-Satz „sourct … mit `source_module()`" wieder wörtlich. Läuft direkt nach dem Merge von Stufe 2, vor 3.1. **Erledigt 26.09., PR #245.**
