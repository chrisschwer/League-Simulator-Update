...
fn test_exact_elo_compatibility_with_r() {
    ...
    let test_data: EloTestData = serde_json::from_str(&data).expect("Failed to parse test data");

    println!(
        "Running {} ELO compatibility tests against R implementation",
        test_data.test_cases.len()
    );
