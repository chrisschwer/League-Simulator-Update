...
shiny_scheduler <- function(update_interval = 300, # 5 minutes in seconds
      ...
      for (league in names(time_windows)) {
        ...
        if (days_ahead == 0) {
          ...
          if (wait_hours * 3600 < min_wait) {
            ...
            next_window_time <- paste(future_date, sprintf(
              "%02d:%02d",
              floor(window$start),
  ...
  cat(paste0(
    "[", Sys.time(), "] Starting Shiny scheduler (update interval: ",
  ...
  while (TRUE) {
    if (is_any_league_active()) {
      ...
      cat(paste0(
        "[", Sys.time(), "] Shiny scheduler - Waiting ",
      ...
      cat(paste0(
        "[", Sys.time(), "] Shiny scheduler - No active leagues. Next window: ",
