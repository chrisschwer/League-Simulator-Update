...
leagueSimulator <- function (season, n = 10000,
                             modFactor = 20, homeAdvantage = 65,
                             adjPoints = NA, adjGoals = NA,
                             adjGoalsAgainst = NA, adjGoalDiff = NA)
  
{
  ...
  if (is.na(adjGoalsAgainst)) {
    adjGoalsAgainst <- rep_len (0, numberTeams)
  }
  if (is.na(adjGoalDiff)) {
    adjGoalDiff <- rep_len (0, numberTeams)
  }
  
  results <- simulations (season = schedule, ELOValue = ELOValues,
