...
update_all_leagues_loop <- function(duration = 480, loops = 31, initial_wait = 0,
  ...
  for (i in 1:loops) {
    ...
    if (need_full_fetch) {
      ...
      # unangetastet, und der naechste Loop versucht es erneut -- inklusive
      # des bekannten Fehlpfad-Kostenproblems (kein Sys.sleep(waittime) bei
      # einem Totalausfall, da das einzige Sys.sleep() am Rundenende steht
      # und `next` hier daran vorbeispringt; Issue #204/#190, bewusst NICHT
      # Teil dieses PRs). Isolation greift erst, wenn WENIGSTENS EINE Liga
      # erfolgreich war; ein Totalausfall ist ja gerade der Fall, in dem es
      # nichts zu isolieren gibt.
      if (length(ok_keys) == 0) {
